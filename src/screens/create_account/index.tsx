...
const CreateAccount: React.FC<LogInSignUpProps> = props => {
  ...
  const {name, phoneNo, passwordd, dialCode, setDialCode, setOtpPassword} =
  ...
  const postSignUpApi = async () => {
    console.log();
    try {
      const response: Root = await (
        await axios.post('/api/v1/users/sign-up/mobile', signIn)
      ).data;
        ...
        <View style={styles.innerContainer}>
          ...
          <View style={styles.phoneNoTextField}>
            <View style={styles.innerPhoneNoTextField}>
              ...
              <Pressable
                style={styles.innerPhonenoTextFieldPressable}
            ...
            <View style={styles.innerPhonenoTextFieldView}>
              <TextInput
                ...
                maxLength={10}
                onChangeText={useConx.setPhoneNo}
          ...
          <View style={[styles.checkBoxContainer]}>
            <Pressable
              style={[
                ...
                {backgroundColor: isPressed ? '#6A1B1E' : 'white'},
              ]}
              onPress={handlePress}>
              {check ? (
                <View>
          ...
          <View>
            <Text style={styles.para}>
              ...
              personal information will be collected,and stored,processed in the
              United States and the European Union on behalf of
              sparity,Properties,Inc
            </Text>
          </View>
          <View style={styles.signInTextView}>
            <Text style={styles.signInText}>Existing User?</Text>
            <Pressable style={styles.signInTextBtn} onPress={handleNavigation}>
              <Text style={styles.signInTextBtn_}> Sign In</Text>
            </Pressable>
          </View>
        </View>
      </View>
      <Modal transparent={false} visible={showModal}>
        <View style={styles.modalMainContainer}>
          <FlatList
            ...
            renderItem={({item}) => (
              <Pressable
                style={styles.modalMainContainerBtn}
                onPress={() => onClick(item)}>
                <Text style={styles.modalMainContainerText}>
                  {item.dial_code}
                </Text>
              </Pressable>
            )}
